...
module.exports = {
  ...
  create(context) {
    ...
    const isSinonImport = (node) => {
      try {
        return node.source.value === "sinon";
      } catch (e) {
        return false;
      }
    };
    ...
    return {
      ImportDeclaration(node) {
        if (!isSinonImport(node)) return;

        // namespaceSpecifier is us importing like import * as sinon from "sinon";
        ...
        if (namespaceSpecifier) {
          const declaredVariables =
            context.getDeclaredVariables(namespaceSpecifier);
        }
      },
