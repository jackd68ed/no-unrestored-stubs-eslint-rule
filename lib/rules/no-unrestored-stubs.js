...
module.exports = {
  ...
  create(context) {
    ...
    const initIsPropertyOfSinon = (node) =>
      node.init.callee.object && node.init.callee.object.name === "sinon";

    const initIsStubFunction = (node) =>
      node.init.callee.property && node.init.callee.property.name === "stub";
    ...
    return {
      ...
      ExpressionStatement(node) {
        if (
          ...
          const stubId = node.expression.callee.object;
